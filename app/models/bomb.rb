class Bomb < ActiveRecord::Base
  ...
  def explode(curTime)
    ...
    self.usersInRange.each do |user|
      ...
      user.hitWith(damage)
      user.notify(sprintf("You just got hit by a bomb which was thrown by %s.  This did %d damage to you and you now have %d hitpoints left", User.find(self.owner_id).name, damage*USER_HITPOINTS,user.hp*USER_HITPOINTS))
    end
